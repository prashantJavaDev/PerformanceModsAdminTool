...
public class OrderDaoImpl implements OrderDao {
    ...
    public Order getorderBymarketplaceOrderID(String marketplaceOrderId) {
        String sqlString = "SELECT tor.*,tm.`MARKETPLACE_NAME`,tw.`WAREHOUSE_NAME`, u1.`USERNAME`as pby,u.`USERNAME` as tby, tal.`CURRENT_COMMISSION` FROM tesy_order tor"
                + " LEFT JOIN tesy_marketplace tm ON tor.`MARKETPLACE_ID`=tm.`MARKETPLACE_ID`"
                + " LEFT JOIN tesy_warehouse tw ON tw.`WAREHOUSE_ID`=tor.`WAREHOUSE_ID`"
                + " LEFT JOIN tesy_available_listing tal ON tal.`SKU`=tor.`MARKETPLACE_SKU`"
    ...
    public List<ProductDetails> getProductListOnMarketplaceSku(String marketplaceSku) {
        ...
        sql.append("SELECT tw.`WAREHOUSE_ID`,tw.`WAREHOUSE_NAME`,tpi.`LARGE_IMAGE_URL`,tcwp.`WAREHOUSE_IDENTIFICATION_NO`,"
                ...
                + " LEFT JOIN tesy_mpn_sku_mapping tmsp ON tmsp.`MANUFACTURER_MPN`=tcwp.`MPN`"
    ...
    public List<Order> getorderTransBymarketplaceOrderID(String marketplaceOrderId) {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT tor.*,tm.`MARKETPLACE_NAME`,tw.`WAREHOUSE_NAME`, u.`USERNAME` AS trackBy, u.`USERNAME` AS processBy FROM tesy_order_trans tor"
                + " LEFT JOIN tesy_marketplace tm ON tor.`MARKETPLACE_ID`=tm.`MARKETPLACE_ID`"
                + " LEFT JOIN tesy_warehouse tw ON tw.`WAREHOUSE_ID`=tor.`WAREHOUSE_ID`"
                + " LEFT JOIN user u ON u.`USER_ID`=tor.`TRACKING_BY`"
    ...
    public void assignWarehouseToOrder(int warehouseId, String orderId) {
        ...
        sqlString = "UPDATE tesy_order tso SET tso.`WAREHOUSE_ID`=?, tso.`PROCESSED_BY`=?, tso.`PROCESSED_DATE`=? WHERE tso.`MARKETPLACE_ORDER_ID`=?";
        params = new Object[]{warehouseId, curUser, curDate, orderId};
        this.jdbcTemplate.update(sqlString, params);

        String sqlString1;
        sqlString1 = "UPDATE tesy_order_trans tot SET tot.`WAREHOUSE_ID`=?, tot.`PROCESSED_BY`=?,tot.`PROCESSED_DATE`=? WHERE tot.`MARKETPLACE_ORDER_ID`=?";
    ...
    public int getProcessOrderCount(int warehouseId, String currentDate) {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM tesy_order tor WHERE tor.`ORDER_ID`");
        Map<String, Object> params = new HashMap<String, Object>();
        if (warehouseId != 0) {
    ...
    public List<ProcessingSheet> getProcessOrderList(int warehouseId, String currentDate) {
        StringBuilder sql = new StringBuilder("SELECT tw.`WAREHOUSE_ID`,tw.`WAREHOUSE_NAME`,tm.`MANUFACTURER_NAME`,twpm.`WAREHOUSE_MPN`,tor.`QUANTITY_SHIPPED`,tcwp.`PRICE`,tor.`SHIP_TO_NAME`,tor.`SHIPPING_ADDRESS_LINE1`,tor.`SHIPPING_ADDRESS_LINE2`,tor.`SHIPPING_ADDRESS_LINE3`,tor.`CITY`,tor.`STATE`,tor.`POSTAL_CODE`,tor.`SHIPPING_PHONE_NO`"
                ...
                + " LEFT JOIN tesy_warehouse tw ON tw.`WAREHOUSE_ID`=tcwp.`WAREHOUSE_ID`"
                + " LEFT JOIN tesy_warehouse_product_mpn twpm ON twpm.`PRODUCT_ID`=tcwp.`PRODUCT_ID`"
                + " LEFT JOIN tesy_order tor ON tor.`WAREHOUSE_ID`=tw.`WAREHOUSE_ID`"
                + " LEFT JOIN  tesy_manufacturer tm ON tm.`MANUFACTURER_ID`=tmsp.`MANUFACTURER_ID`"
    ...
    public int addOrderTracking(Order order, int warehouseId) {
        ...
        int curUser = ((CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUserId();
        String curDate = DateUtils.getCurrentDateString(DateUtils.IST, DateUtils.YMDHMS);
        sqlString = "UPDATE tesy_order SET TRACKING_ID=?, TRACKING_CARRIER=?, TRACKING_BY=?, TRACKING_DATE=?, WAREHOUSE_TRACKING_STATUS =1, WAREHOUSE_ORDER_STATUS =1 WHERE MARKETPLACE_ORDER_ID=? AND WAREHOUSE_ID =?";
    ...
    public List<Listing> getMarketplaceSkuList() {
        String sqlString = "SELECT 'ATVPDKIKX0DER' AS Marketplace_Id,'SellerSKU' AS ID_type,tal.`SKU`,tal.`LAST_LISTED_PRICE`,tal.`ISAMAZONFULLFILLED` FROM tesy_available_listing tal WHERE tal.`FEED_STATUS`= '0' LIMIT 20 ";
    ...
    public void loadBulkOrderTrackingDataLocally(String path, int marketplaceId) {
        try {
            int curUser = ((CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal()).getUserId();
            String curDate = DateUtils.getCurrentDateString(DateUtils.IST, DateUtils.YMDHMS);
            String sql = "TRUNCATE TABLE `tel_easy_admin_tool`.`tesy_temp_bulk_tracking`";
            this.jdbcTemplate.update(sql);
            LogUtils.systemLogger.info(LogUtils.buildStringForLog("Truncate tesy_temp_bulk_tracking done.", GlobalConstants.TAG_SYSTEMLOG));
            //query load data from bulk order tracking csv file into tesy_temp_bulk_tracking
            sql = "LOAD DATA LOCAL INFILE '" + path + "' INTO TABLE `tel_easy_admin_tool`.`tesy_temp_bulk_tracking` CHARACTER SET 'latin1' FIELDS ESCAPED BY '\"' TERMINATED BY ',' LINES TERMINATED BY '\n' IGNORE 1 LINES (`WAREHOUSE_NAME`,`ORDER_ID`, `TRACKING_ID`,`TRACKING_CARRIER`)";
            ...
            String sql3 = "UPDATE tesy_temp_bulk_tracking tb\n"
                    + "LEFT JOIN tesy_warehouse tw ON tb.`WAREHOUSE_NAME`=tw.`WAREHOUSE_NAME`\n"
                    + "SET tb.`WAREHOUSE_ID` = tw.`WAREHOUSE_ID`\n"
                    + "WHERE tb.`WAREHOUSE_NAME`=tw.`WAREHOUSE_NAME`";
            int d = this.jdbcTemplate.update(sql3);

            String sql1 = "UPDATE tesy_order tr \n"
                    + "LEFT JOIN tesy_temp_bulk_tracking tb ON tr.`MARKETPLACE_ORDER_ID` = tb.`ORDER_ID`\n"
            ...
            String sql2 = "INSERT INTO tesy_order_trans SELECT   \n"
                    ...
                    + "FROM tesy_order tr LEFT JOIN  tesy_temp_bulk_tracking tb ON tb.`ORDER_ID` = tr.`MARKETPLACE_ORDER_ID`\n"
                    + "WHERE tr.`MARKETPLACE_ORDER_ID` = tb.`ORDER_ID` AND tr.`MARKETPLACE_ID`=?;";
    ...
    public List<String> getMarketplaceSkuListForFees() {
        String sql = "SELECT tal.`SKU` FROM tesy_available_listing tal LIMIT 5";
    ...
    public List<Order> getOrderListOfUnshippedOrderForDashboard(int count) {
        try {
            StringBuilder sql = new StringBuilder();
            sql.append("SELECT t.`MARKETPLACE_ORDER_ID`,t.`ORDER_DATE`,DATE_ADD(t.`ORDER_DATE`, INTERVAL 2 DAY) shippingdate FROM tesy_order t  \n"
                    + "WHERE DATEDIFF(NOW(),t.`ORDER_DATE`)>2 AND t.`ORDER_STATUS`='Unshipped'");
            if (count == 1) {
                sql.append(" LIMIT 10");
            }

//            String sql = "SELECT t.`MARKETPLACE_ORDER_ID`,t.`ORDER_DATE`,DATE_ADD(t.`ORDER_DATE`, INTERVAL 2 DAY) shippingdate FROM tesy_order t  \n"
//                    + "WHERE DATEDIFF(NOW(),t.`ORDER_DATE`)>2 AND t.`ORDER_STATUS`='Unshipped' LIMIT 10;";
    ...
    public List<Order> getOrderListForTrackingUpdate() {
        try {
            String sql = "SELECT * FROM tesy_temp_bulk_tracking";
            return this.jdbcTemplate.query(sql.toString(), new RowMapper<Order>() {
                @Override
                public Order mapRow(ResultSet rs, int i) throws SQLException {
                    Order order = new Order();
    ...
    public void updateTrackingRecord(String trackingID) {
        String sql = "UPDATE tesy_order tor SET tor.`TRACKING_STATUS`=1 WHERE tor.`TRACKING_ID`=?";
...
//                    + "tcp.`WAREHOUSE_IDENTIFICATION_NO`,\n"
//                    + "tcp.`PRICE`\n"
//                    + "FROM tesy_order tor LEFT JOIN tesy_mpn_sku_mapping msm ON msm.`SKU` = tor.`MARKETPLACE_SKU`\n"
//                    + "LEFT JOIN tesy_current_warehouse_product tcp ON tcp.`WAREHOUSE_IDENTIFICATION_NO` = msm.`MANUFACTURER_MPN` LIMIT 1";
//
//            @Override
//            public Order mapRow
...
//    }
    ...
    public String getPoNumberBymarketplaceOrderID(String marketplaceOrderId) {
        String sql = "SELECT tor.`PO_NUMBER` FROM tesy_order tor WHERE tor.`MARKETPLACE_ORDER_ID`=? LIMIT 1";
        return this.jdbcTemplate.queryForObject(sql, String.class, marketplaceOrderId);
    }

    @Override
    public String getMaxPoNumberForAutogeneration() {
        String sql = "SELECT MAX(tor.`PO_NUMBER`) FROM tesy_order tor";
    ...
    public String getOrderStatusOnMarketplaceOrderId(String marketplaceOrderId) {
        String sql = "SELECT tor.`ORDER_STATUS` FROM tesy_order tor WHERE tor.`MARKETPLACE_ORDER_ID`=?";
        return this.jdbcTemplate.queryForObject(sql, String.class, marketplaceOrderId);
    }
    ...
    public void updateOrderAcknowledgementRecord(String orderId) {
        String sql = "UPDATE tesy_order tor SET tor.`MARKETPLACE_AKNOWLEDGE_STATUS`=1 WHERE tor.`MARKETPLACE_ORDER_ID`=?";
    ...
    public Order getOrderForAmazonAcknowledgement() {
        try {
            String sql = "SELECT tor.`MARKETPLACE_ORDER_ID`,tor.`ORDER_ITEM_ID` FROM tesy_order tor \n"
    ...
    public void loadMarketPlaceOrderDataLocally(String path, int marketplaceId) {
        try {
            ...
            sql = "INSERT IGNORE INTO tesy_order(`MARKETPLACE_ID`,`LAST_UPDATED_DATE`,`MARKETPLACE_ORDER_ID`,`MARKETPLACE_SKU`,`MARKETPLACE_LISTING_ID`,`ORDER_DATE`,`SHIP_BY_DATE`,`DELIVERY_BY_DATE`,`CUSTOMER_NAME`,`CUSTOMER_PHONE_NO`,`QUANTITY_UNSHIPPED`,`ORDER_ITEM_ID`,`PRICE`,`TAX`,`SHIPPING`,`SHIP_TO_NAME`,`SHIPPING_ADDRESS_LINE1`,`SHIPPING_ADDRESS_LINE2`,`SHIPPING_ADDRESS_LINE3`,`CITY`,`STATE`,`POSTAL_CODE`,`COUNTRY`,`SHIPPING_PHONE_NO`,`FULFILLMENT_CHANNEL`,`ORDER_STATUS`,`UPDATED_BY`)\n"
    ...
    public int getOrderCount(int marketplaceId, String marketplaceOrderId, String poNumber, String customerName, String marketplaceSku, String marketplaceListingId, String orderStatus, String fulfillmentChannel, String startDate, String stopDate) {
        StringBuilder sql = new StringBuilder("SELECT COUNT(*) FROM tesy_order teo"
                + " LEFT JOIN tesy_marketplace tm ON teo.`MARKETPLACE_ID`=tm.`MARKETPLACE_ID`"
